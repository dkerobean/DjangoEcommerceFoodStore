...
from .models import Category, Tag, Review, Address, Order
...
def user_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            activation_link = f"http://{current_site.domain}/activate/{uidb64}/{token}/"  # noqa

            mail_subject = 'Activate your account'
            message = render_to_string('frontend/auth/confirmation_email.html', {         # noqa
                'user': user,
            ...
            messages.success(
                request, 'Registration successful. Please check your email to activate your account.')      # noqa
...
def activate_user_account(request, uidb64, token):
    try:
        ...
        user_id = int_value = int.from_bytes(uid, byteorder='big')    # noqa
...
def view_product(request, pk):
    ...
    if request.method == 'POST':
        ...
        if existing_review:
            messages.error(
                request, 'You have already submitted a review for this product')   # noqa
            return redirect('view-product', product_id)

        user_review = Review.objects.create(review_text=review,
                                            review_title=title,
...
def shop_page(request):
    ...
    ratings = {}  # Create an empty dictionary to store average ratings for each product    # noqa

    for product in products:
        # Retrieve the average rating for the product
        average_rating = product.product_reviews.aggregate(
                            Avg('rating'))['rating__avg']

        # Round the average rating to 2 decimal places
        average_rating = round(average_rating, 2) if average_rating else None

        # Store the average rating in the ratings dictionary with the product's ID as the key   # noqa
...
def filter_products(request, q):
    ...
    if query:
        ...
        products = products.filter(category__name__icontains=query) | products.filter(tag__name__icontains=query)  # noqa
